                    ...
                    <div class="row">
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <input name="field[{{ $item['id'] }}]" id="{{ $item['id'] }}"
                                    class="form-control @error($item['id']) is-invalid @enderror" type="text" autocomplete="false"
                                    value="{{ $item['value'] }}">
                                @error($item['id'])
                ...
                @elseif($item['type'] == 1)
                    <div class="row">
                        <div class="col">
                            <div class="mb-3">
                                <label>{{ $item['label'] }}</label>
                                <textarea name="field[{{ $item['id'] }}]"
                                    class="summernote @error($item['id'])@endif" rows="3">{{ $item['value'] }}</textarea>
                   @error($item['id'])
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
